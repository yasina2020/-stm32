...
{  
	...
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_4|GPIO_Pin_1|GPIO_Pin_15|GPIO_Pin_0;
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP;  //推挽输出
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
	GPIO_Init(GPIOA, &GPIO_InitStructure); //GPIOA	
	GPIO_SetBits(GPIOA,GPIO_Pin_4|GPIO_Pin_1|GPIO_Pin_15|GPIO_Pin_0);
...
//EndX,  EndY:  起点坐标  
//*pic:         图片的起始地址
void LCD_ShowPicture(u16 StartX, u16 StartY, u16 EndX, u16 EndY, const char *pic)
{
    ...
    for(u16 Y=StartY;Y<=EndY;Y++)
    {
        ...
        {
            POINT_COLOR=(unsigned short)pic[counter] | pic[counter+1]<<8; //设置颜色
            LCD_DrawPoint(X, Y);
            counter+=2;
        }
    }
